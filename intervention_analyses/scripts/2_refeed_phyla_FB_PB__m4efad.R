...
FB_summary_refeed <- FBratio_refeed %>%
  group_by(Age_months) %>%
  summarise(
    mean_firmicutes = mean(p__Firmicutes, na.rm = TRUE),
    sd_firmicutes = sd(p__Firmicutes, na.rm = TRUE),
    mean_bacteroidetes = mean(p__Bacteroidetes, na.rm = TRUE),
    sd_bacteroidetes = sd(p__Bacteroidetes, na.rm = TRUE),
    F_B_ratio = mean_firmicutes / mean_bacteroidetes,
    sd_F_B_ratio = sd(p__Firmicutes / p__Bacteroidetes, na.rm = TRUE),
    .groups = "drop"
  )
...
PB_summary_refeed <- PBratio_refeed %>%
  group_by(Age_months) %>%
  summarise(
    mean_prevotella = mean(g__Prevotella, na.rm = TRUE),
    sd_prevotella = sd(g__Prevotella, na.rm = TRUE),
    mean_bacteroides = mean(g__Bacteroides, na.rm = TRUE),
    sd_bacteroides = sd(g__Bacteroides, na.rm = TRUE),
    P_B_ratio = mean_prevotella / mean_bacteroides,
    sd_P_B_ratio = sd(g__Prevotella / g__Bacteroides, na.rm = TRUE),
    .groups = "drop"
  )
