...
# Calculate prevalence by Condition group and identify the species that are >85% prevalent
...
# Select the species for both Conditions 
# Define the core species
core_species <- c(
...
# Get prevalence for all Condition + Taxa combinations
prevalence_all <- species_long %>%
  group_by(Condition, Taxa) %>%
  summarise(prevalence = mean(RA > 0), .groups = "drop")

# Filter only for those core species
prevalence_filtered <- prevalence_all %>%
  filter(Taxa %in% core_species) %>%
  arrange(Taxa, Condition)


# Clean species names
core_species_by_group <- prevalence_filtered %>%
...
prevalence_filtered
 ...
 ggplot(prevalence_filtered, aes(x = prevalence, y = reorder(Taxa, prevalence), fill = Condition)) +
  geom_bar(stat = "identity", position = "dodge", color = "black", linewidth = 0.2) +
  scale_fill_manual(values = c("MAM" = "darkgoldenrod1", "Healthy" = "cyan4")) +
  labs(
    x = "Prevalence",
